...
def synchronous_times(index, n_samples, seed, rate_scale=1.0, synchrony=0.0):
    """Feuerzeiten mit Synchronität: Neuron 0 feuert wie gewohnt; jedes andere Neuron übernimmt einen Anteil `synchrony` der Spikes von Neuron 0 (gleiche Startzeiten) und behält nur den Anteil 1 - synchrony
    seiner eigenen. Die Refraktärzeit bleibt eingehalten. Bei synchrony = 0 identisch zu `firing_times` (die Quellen sind dann unabhängig, sonst abhängig)."""
    base = firing_times(index, n_samples, seed, rate_scale)
    if index == 0 or synchrony <= 0:
        return base
    lead = firing_times(0, n_samples, seed, rate_scale)
    rng = np.random.default_rng([seed, 500 + index])
    take = lead[rng.random(len(lead)) < synchrony]
    own = base[rng.random(len(base)) >= synchrony]
    kept = []
    for t in np.sort(np.concatenate([take, own])):
        if not kept or t - kept[-1] >= C.REFRACTORY:
            kept.append(int(t))
    return np.array(kept, dtype=int)


def neuron_source(index, n_samples, seed, rate_scale=1.0, similarity=1.0, jitter=0.0, synchrony=0.0):
    starts = synchronous_times(index, n_samples, seed, rate_scale, synchrony)
...
def make_dataset(n_neurons, n_electrodes, rate_scale, similarity, jitter, noise, n_samples, seed, synchrony=0.0):
    m, n = n_neurons, n_electrodes
    sources, raw, starts_list = [], [], []
    for i in range(m):
        s, starts = neuron_source(i, n_samples, seed, rate_scale, similarity, jitter, synchrony)
        raw.append(s)
        sources.append(s)
